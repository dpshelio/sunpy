...
class HeliographicCarrington(HeliographicStonyhurst):
    ...
    def sol_string(self, form='minimal'):
        ...
        sol_str = 'SOL{:%Y-%m-%d}'.format(datetime)
        ...
        if form == 'minimal':
            ...
            sol_str += 'T{date:%H:%M%S}L{lon:03.0f}C{colat:03.0f}'.format(date=datetime,
                                                                          lon=lon.value,
                                                                          colat=colat.value)
            if form == 'full':
                # FIXME: not sure about the precision
                sol_str += 'R{rad:f}'.format(rad=self.radius.to(u.m) / RSUN_METERS)
        else:
